...
```{r}
states<-map_data("state") %>% filter(region %in% c("california","oregon","nevada","utah"))
...
```{r}
ggplot() +  geom_polygon(data=sgbdf,aes(x=long, y=lat, group=group), fill="white", color="black", alpha=0) +
  geom_polygon(data=states,aes(x=long, y=lat, group=group), fill="white", color="black", alpha=0 ) +
