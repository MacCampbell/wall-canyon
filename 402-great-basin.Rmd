...
nwlakes<-sordf %>% mutate(MeanX=mean(long), MeanY=mean(lat), Label="Oregon Closed Basins") %>%
  dplyr::select(MeanX, MeanY, Label) %>% unique()
```
...
ggplot() + 
  ...
  scalebar(sordf, dist = 100, dist_unit = "km",
...
```{r}
save(sordf, file="spatial-data/sordf.rda")
save(sgbdf, file="spatial-data/sbgdf.rda")
save(cadf, file="spatial-data/cadf.rda")
save(codf, file="spatial-data/codf.rda")
```
...
mapg<-ggplot() + 
  ...
  theme(axis.title=element_text(size=14)) +
   scalebar(dist = 200, dist_unit = "km",
             transform = TRUE, model = "WGS84", x.min=min(rast.table$x), x.max=max(rast.table$x), y.min=min(rast.table$y), y.max=max(rast.table$y), location = "bottomleft")
...
mapp<-ggplot() + 
  geom_raster(data = rast.table, mapping = aes(x = x, y = y), fill = rast.table$rgb, interpolate = TRUE)  +
  geom_holygon(data=pluvdf,aes(x=long, y=lat, group=group), size=1, fill="cornflowerblue", alpha=.75) +
  geom_polygon(data=sordf,aes(x=long, y=lat, group=group), fill="white", color="black", alpha=0) +
  geom_polygon(data=sgbdf,aes(x=long, y=lat, group=group), fill="white", color="black", alpha=0) +
  geom_polygon(data=cadf,aes(x=long, y=lat, group=group), fill="white", color="black", alpha=0) +
  geom_polygon(data=codf,aes(x=long, y=lat, group=group), fill="white", color="black", alpha=0) +
  ...
  theme(axis.title=element_text(size=14)) +
     scalebar(dist = 200, dist_unit = "km",
             transform = TRUE, model = "WGS84", x.min=min(rast.table$x), x.max=max(rast.table$x), y.min=min(rast.table$y), y.max=max(rast.table$y))
